...
import type { AssignmentInput, GeneratedAssessment, AssessmentSection, Question, QuestionType, Difficulty } from "../types/index"
...
function buildPrompt(input: AssignmentInput): string {
  const sectionDescriptions = input.sections.map(s => `
...
function parseAndValidate(raw: string, input: AssignmentInput): GeneratedAssessment {
  ...
  const sections: AssessmentSection[] = parsed.sections.map((rawSection: any, sIdx: number) => {
    ...
    if (rawSection.questions.length !== config.questionCount) {
      rawSection.questions = rawSection.questions.slice(0, config.questionCount)
    }

    const questions: Question[] = rawSection.questions.map((q: any, qIdx: number) => {
      if (!q.text || typeof q.text !== "string") {
        throw new Error(`Question ${qIdx + 1} in Section ${rawSection.name} has no text`)
      }

      const validDifficulties: Difficulty[] = ["easy", "medium", "hard"]
      const difficulty: Difficulty = validDifficulties.includes(q.difficulty) ? q.difficulty : "medium"

      const validTypes: QuestionType[] = ["mcq", "short_answer", "long_answer", "true_false", "fill_blank"]
      const type: QuestionType = validTypes.includes(q.type) ? q.type : config.questionType

      if (type === "mcq") {
        if (!q.options || q.options.length !== 4) {
          throw new Error(`MCQ question "${q.text.slice(0, 30)}..." must have exactly 4 options`)
        }
      }

      return {
        id: q.id || `${rawSection.name?.replace("Section ", "")}${qIdx + 1}`,
        number: qIdx + 1,
    ...
    while (questions.length < config.questionCount) {
      ...
      questions.push({
        ...
        options: config.questionType === "mcq" ? [
  ...
  return {
    ...
    totalMarks: typeof parsed.totalMarks === "number" ? parsed.totalMarks : input.totalMarks,
    ...
    generalInstructions: Array.isArray(parsed.generalInstructions)
      ...
      : ["Attempt all questions", "All answers must be written in the answer sheet"],
    sections,
...
function generateMockAssessment(input: AssignmentInput): GeneratedAssessment {
  const sections: AssessmentSection[] = input.sections.map((sec) => {
    ...
    const mediumCount = Math.round(totalCount * (sec.difficultyMix.medium / 100))
    ...
    for (let i = 0; i < totalCount; i++) {
      ...
      if (sec.questionType === "mcq") {
        if (lowerSubject.includes("math") || lowerSubject.includes("algebra") || lowerSubject.includes("calc")) {
          ...
          answer = "B"
        } else if (lowerSubject.includes("science") || lowerSubject.includes("physics") || lowerSubject.includes("chem") || lowerSubject.includes("bio")) {
        ...
        answer = "Comprehensive discussion about origin, concepts, and challenges."
      }

      // If text from PDF is provided, blend it in creatively!
      if (input.uploadedFileText) {
        text = `[Reference Material Text Base] ` + text + ` Reference text context snippet: "${input.uploadedFileText.trim().substring(0, 80)}..."`
      }
...
export async function generateAssessment(
  ...
  if (isMockMode()) {
    onProgress?.("OpenRouter key not detected. Initializing creative mock fallback...")

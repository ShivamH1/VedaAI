...
export default function AssessmentGenerationProgress({ params }: PageProps) {
  ...
  useEffect(() => {
    if (assessmentId && currentType === "generation_complete") {
      const timer = setTimeout(() => {
        router.push(`/assessment/${id}/result?assessmentId=${assessmentId}`);
      }, 1000);
      return () => clearTimeout(timer);
    }
  }, [assessmentId, currentType, id, router]);
    ...
    <div className="flex min-h-screen bg-[#ececee] text-text-primary">
      ...
      <div className="flex-1 flex flex-col min-w-0 p-3 lg:pl-0 h-screen overflow-y-auto pb-24 lg:pb-3">
        ...
        <main className="flex-1 flex flex-col items-center justify-center bg-white rounded-custom-xl border border-border-subtle shadow-premium p-6 md:p-12 text-center min-h-[500px]">
          {error ? (
            <div className="flex flex-col gap-5 items-center justify-center max-w-[480px] w-full py-8">
              <div className="w-16 h-16 bg-red-50 text-red-600 rounded-full flex items-center justify-center border border-red-200 shadow-sm animate-bounce">
                <span className="text-2xl font-bold">✕</span>
              </div>
              <div className="flex flex-col gap-2">
                <h3 className="text-xl font-bold text-text-primary">Generation Failed</h3>
            ...
            <div className="max-w-[480px] w-full flex flex-col items-center gap-6">
              ...
              <div className="w-full flex flex-col gap-2.5 border-t border-border-subtle pt-6 mt-4 text-left">
                {[
                  { label: "Queued Assessment Task", done: progress >= 10 },
                  { label: "Analyzed Subject & Blueprint", done: progress >= 40 },
                  { label: "Generated Question Formulations", done: progress >= 80 },
